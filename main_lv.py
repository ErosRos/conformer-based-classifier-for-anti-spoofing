...
def produce_evaluation_file(dataset, model, device, save_path):
    data_loader = DataLoader(dataset, batch_size=4, shuffle=False, drop_last=False, collate_fn=my_collate)
    model.eval()
    
    fname_list = []
    score_list = []
    text=open(save_path,'w')
    text.close()
...
if __name__ == '__main__':
    ...
    assert args.n_average_model<args.n_mejores_loss+1, 'average models must be smaller or equal to the number of saved epochs'
    ...
    model_tag = 'ConfLV_{}_{}_{}_ES{}_H{}_NE{}_KS{}'.format(
